...
RSpec.describe Prog::Storage::ArchiveVm do
  ...
  let(:vm) {
    vm = create_vm(vm_host_id: vm_host.id, umi: true)
  ...
  describe "#assemble" do
    ...
    context "when vm exists" do
      ...
      it "fails because vm is not configured for UMI creation" do
        vm.update(umi: false)
        expect {
          described_class.assemble(vm.id)
        }.to raise_error("Vm is not configured for UMI creation")
      end
  ...
  describe "#wait_archive" do
    ...
    it "cleans up and pops on success" do
      ...
      expect { prog.wait_archive }.to hop("create_machine_image")
    end
  ...
  describe "#create_machine_image" do
    it "creates a machine image and pops" do
      expect { prog.create_machine_image }.to exit({"msg" => "Machine Image created successfully"})
        .and change(MachineImage, :count).by(1)

      mi = MachineImage.last
      expect(mi.project_id).to eq(vm.project_id)
      expect(mi.location_id).to eq(vm.location_id)
      expect(mi.name).to start_with("#{vm.boot_image}-")
      expect(mi.bucket_prefix.length).to eq(15)
    end
  end
